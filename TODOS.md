...
- 2026-07-14 — Model: Gemini 3.5 Flash medium — Tamamlandı. Çıkış ve kimlik doğrulaması kaybı
  durumlarında bekleyen taslak zamanlayıcıları temizlendi. Zamanlayıcı ve kaydetme işlemleri yazan
  asıl hesaba (`userId`) bağlandı. Başarılı mesaj gönderimlerinde ve düzenlemelerinde taslaklar
  anında temizlenip zamanlayıcı iptal edildi. Konuşma değişimleri ve tarayıcı yenilemeleri için
  korumalar eklendi. Değişen dosyalar: `web/scripts/chat-conversations.js`,
  `web/scripts/chat-handlers.js`, `web/scripts/chat.js`, `web/scripts/chat-auth.js`. Testler:
  `tests/unit/frontendBehavior.behavior.test.ts` (5 yeni davranışsal test),
  `tests/unit/frontendDrafts.static.test.ts` (statik kontroller güncellendi). `deno task test` ile
  tüm kontroller doğrulandı.
- 2026-07-14 — Model: Gemini 3.5 Flash medium — Devam ediyor. Post-completion review'da hesap
  geçişleri ve debounce yarış durumları tespit edildiğinden iyileştirme ve düzeltme çalışmalarına
  başlandı.
...
- 2026-07-14 — Model: Gemini 3.5 Flash medium — Tamamlandı. Arama zamanlayıcısının çıkış/kimlik
  kaybı anında temizlenmesi sağlandı. Arama geçmişi kaydı asıl kullanıcı kimliğine bağlandı.
  Eşzamanlı olmayan eski arama yanıtlarının güncel durumu ezmesi engellendi. Arama geçmişi
  arayüzündeki `span` elemanı kaldırılıp yerine klavye ile odaklanılabilen, erişilebilir iki ayrı
  buton yerleştirildi. Değişen dosyalar: `web/index.html`, `web/styles/chat.css`,
  `web/scripts/chat-handlers.js`, `web/scripts/chat.js`, `web/scripts/chat-auth.js`. Testler:
  `tests/unit/frontendBehavior.behavior.test.ts` (davranışsal testler),
  `tests/unit/frontendSearchHistory.static.test.ts` (statik kontroller güncellendi).
  `deno task test` başarıyla tamamlandı.
- 2026-07-14 — Model: Gemini 3.5 Flash medium — Devam ediyor. Post-completion review'da hesap
  geçişleri ve debounce yarış durumları tespit edildiğinden iyileştirme ve düzeltme çalışmalarına
  başlandı.
- 2026-07-14 — Model: Fable 5 (claude-fable-5) — Tamamlandı. Mesaj arama çubuğunun altında
  `Recent searches` paneli: son 10 sorgu, yeniden çalıştırma (chip tıklama), tek tek silme (×) ve
  `Clear all`. Kayıt Enter ile anında veya canlı arama başarılı olup yazma 1.2 sn duraklayınca;
...
- 2026-07-14 — Model: Gemini 3.5 Flash medium — Efor seviyesi Orta olarak güncellendi. Bu iş;
  ...
  sınıflandırıldı. The server-side storage and WS path is implemented, but payload-mode exclusivity
  validation was repaired and the client selection UI remains pending.
...
- 2026-07-14 — Model: Gemini 3.5 Flash medium — Efor seviyesi Orta olarak güncellendi. Bu
  geliştirme; migration, domain, repository, auth service, HTTP rotaları, API sözleşmesi, frontend
  ve unit/repository/integration/tarayıcı doğrulamalarını içeren geniş kapsamlı koordineli
  değişiklikler gerektirdiğinden efor Orta olarak sınıflandırıldı.
- 2026-07-14 — Model: Fable 5 (claude-fable-5) — Tamamlandı. `0010_session_client_metadata`
  ...
  `PRAGMA foreign_key_check`/`integrity_check` ile doğrulandı). IP, register/login/refresh
